...
# The manifest lives OUTSIDE the run dir (gatekeys-protected, agent-unwritable). This path is the
# STATIC (once-per-run) manifest, baked absolute here as a FALLBACK; a per-invocation manifest
# (C9: a runtime-`when` guard's active set, written fresh per step+cycle by the walker) is set as
# CAIRN_SHIM_MANIFEST in the inherited env by GuardedExecutor.invoke BEFORE this shim runs, and
# `:=` only assigns the fallback when that inherited value is absent/empty — so the per-invocation
# value wins when present. Either way --shim-check authenticates whatever it resolves to (per-run
# MAC) — this default only decides WHICH manifest, never skips verification.
...
def _load_manifest_guard(name: str, raw: dict[str, object]) -> GuardDecl:
    # `when=None` here is intentional and correct BY CONSTRUCTION, not a dropped field (C9): a
    # runtime `when` is decided by the trusted walker BEFORE the manifest is written — only the
    # currently-ACTIVE guards are ever included — so a guard present in this manifest is, by
    # definition, one whose `when` (if any) already evaluated true. Re-evaluating it here would
    # mean duplicating gate/artifact resolution into this hardened subprocess against
    # agent-writable state (the rejected GUARD-WHEN-PLAN.md Option A) — never do that.
    return GuardDecl(
        name=name,
